...
import book from '../../resources/img/image_book.png'
...
export const BookList = () => {
    ...
    const booksList = books.map((item)=> {
        ...
        switch (item.status) {
            case 'Забронировать':
                classBtn = 'card__button';
              break;
            case 'занята до 23.04':
                classBtn = 'card__button-booked';
              break;
            case 'занята до 03.05':
                classBtn = 'card__button-booked';
              break;
            case 'Забронирована':
              classBtn = 'card__button-busy';
              break;
          }
        ...
        return (
         <NavLink to={`/books/:category/${item.id}`} key={item.id} >
             <div className="card" data-test-id='card' >
                <img src={item.img ? book : cat} alt="img" className='card__img' />
                 <div className="card__wrapper">
                    <div className="card__score"> {(item.score)? starList : <h2>ещё нет оценок</h2>} </div>
                    <h3 className='card__title'> {item.title}</h3>
                    <h4 className='card__subtitle'>{item.author} </h4>
                    <button className={classBtn} type='button'> {item.status} </button>
              </div>
        </div>
         </NavLink>
        )
